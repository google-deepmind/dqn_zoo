...
# Batch variant of mad_q_learning with fixed tau input across batch.
...
def mad_q_learning(
  ...
  Returns:
    MAD-Q-learning temporal difference error.
  """
  chex.assert_rank([
...
_batch_mad_q_learning = jax.vmap(mad_q_learning)

class MadDqn(parts.Agent):
  ...
  def __init__(
    ...
    def loss_fn(online_params, target_params, transitions, rng_key, mixture_ratio):
      ...
      dist_q_target_tm1 = network.apply(target_params, target_key,
                                      transitions.s_tm1).q_dist
      td_errors = _batch_mad_q_learning(
          dist_q_tm1,
          transitions.a_tm1,
          transitions.r_t,
